...
v0.46.0 完成 Phase 46 Admin Document Delete Flow：Admin / Analyst 可刪除後台文件紀錄與本機 upload / PDF page image artifacts；Viewer 仍被 role guard 擋下。這不是 production deletion governance、restore workflow、Agent destructive tool、Qdrant cleanup API、production approval workflow、audit storage、worker 清理流程、migration 或 deployment 變更。

DocuRAG AgentOps backend MVP v0.46.0 是最小 FastAPI 服務，提供 healthcheck、文件本機上傳、metadata 保存、文件列表、文件詳情、文件刪除、`.txt` direct ingestion、text-native PDF ingestion、OCR mock API、provider-selected OCR API、VLM-first invoice parser spike、OCR / VLM evidence alignment、deterministic invoice parser fallback、parse / fields API、Agent run / lookup API、manual vector indexing API、local RAG query API、demo auth API、built-in RAG eval API、retrieval evaluation runner、FastEmbed rerank adapter、hybrid / `hybrid_rerank` runtime provider、demo seed script 與 API smoke test，並允許 local frontend 透過 CORS 呼叫。v0.27.0 起 backend 採 aggressive demo defaults：`DOCURAG_RAG_RETRIEVAL_PROVIDER=hybrid_rerank`、`DOCURAG_EMBEDDING_PROVIDER=ollama`、`DOCURAG_RERANK_PROVIDER=fastembed`。v0.46.0 新增 `DELETE /documents/{document_id}`，Admin / Analyst 可刪除文件 metadata、本機 upload artifact 與 PDF page image artifact；Viewer 會收到 forbidden。v0.27.1 起 VLM parser request 會同時帶原始圖片與 compact OCR context；欄位值可對回 OCR line / bbox，未命中時會標示 evidence unmatched / unavailable。v0.28.0 已固定 `image_ocr`、`text_upload`、`pdf_text` 與 `pdf_scanned_pending_ocr` 的上傳分流；目前 runtime 已支援 `.txt` 直接建立 `text_upload` chunks，也會用 `pypdf` 抽取 text-native PDF 文字層建立 `pdf_text` chunks。v0.29.0 新增 `POST /eval/rag/built-in`，讓 Admin / Analyst 執行固定 `hybrid_rerank` 的 10 張 synthetic 中文發票 retrieval benchmark；後續 focused hardening 也讓 Ollama VLM adapter 可處理 `response` / `thinking` / fenced JSON、金額字串與 line item alias，並補強 RAG vector stale filtering 與 Ollama generation latency guardrails。v0.31.0 新增 opt-in PostgreSQL metadata repository foundation、local JSON fallback adapter 與 explicit local JSON import command。Phase 32 `32-02` 新增 formal Auth / RBAC PostgreSQL schema foundation 與 explicit migration command；`32-03` 新增 formal signed bearer token guard、project access filtering、Analyst / Admin write guard 與 Viewer forbidden；`32-04` 完成 frontend role surface 與 `v0.32.0` release sync。Phase 33 `33-02` 新增 opt-in Redis backend slice，可在 `DOCURAG_REDIS_URL` 設定且安裝 optional `backend[redis]` 時 best-effort 使用 session cache、RAG query cache 與 rate limit；Redis 未設定、client 未安裝或連線不可用時維持既有 fallback。Phase 33 `33-03` 新增 NATS worker skeleton 與 task status slice：`DOCURAG_NATS_URL=memory://` 可驗證 publish / consume / status update，optional `backend[nats]` 可連真實 NATS，`/tasks` 可讀取 worker task records。v0.33.0 完成 `scripts/worker-demo-smoke.ps1` 與 release sync，可一次驗證 Redis demo path、NATS memory worker skeleton 與 task status API。v0.34.0 完成 scanned PDF OCR baseline：scanned / mixed PDF page images 可透過 provider-selected OCR 產生 `pdf_page_ocr` chunks，並以 smoke 驗證 parser / RAG handoff。v0.35.0 完成 RAG indexing quality release：`POST /documents/{document_id}/index/vector` 可選 `fixed_size` / `semantic` chunking，Qdrant payload index / filter 支援 tenant、project、document 與 source scope，並新增 project reindex 與 stale vector cleanup smoke。v0.36.0 完成 eval dashboard / rerank analysis release：`POST /eval/runs` 可對 eval dataset 執行 keyword、vector、hybrid、vector_rerank、hybrid_rerank strategy comparison，並保存 metrics、failure / fallback cases、trace metadata coverage 與 rerank before / after rank / score。`DOCURAG_AUTH_MODE=demo` 會啟用 demo login / me / logout 與 write API role guard，Admin / Analyst 可操作 ingestion、built-in eval 與文件刪除，Viewer 只能查詢與下載。`/rag/query` 與 Agent `search_documents` 會優先使用 hybrid rerank；Ollama embedding、Qdrant 或 FastEmbed runtime 不可用時，會 fallback 到 keyword evidence 並寫入 trace。LLM provider 未覆寫時仍預設嘗試 Ollama `qwen3.5:4b` generation，Ollama 不可用或回空 final response 時 fallback 到 retrieved chunks。PostgreSQL metadata repository 仍為 opt-in foundation，不代表 production database operation；Phase 32 formal Auth / RBAC boundary 已有 signed bearer guard、project access filtering 與 role-gated frontend surface；Phase 33 Redis + NATS worker demo 也不代表 production async OCR / parser / indexing / eval pipeline 已完成；Phase 34 scanned PDF OCR baseline 也不代表 layout analysis、table reconstruction、human correction 或 production OCR accuracy tuning；Phase 36 eval dashboard 也不代表 LLM-as-judge、answer faithfulness、citation quality scoring、production monitoring trend 或 production indexing worker 已完成。v0.37.0 完成 inference ops / vLLM serving demonstration：OpenAI-compatible provider 可明確啟用，local vLLM / Docker path 與 `scripts/inference-benchmark-smoke.ps1` 可記錄 latency、tokens、throughput、KV cache / GPU memory estimate 或 provider unavailable skip reason。v0.38.0 完成 Agent runtime hardening：`DOCURAG_AGENT_PLANNER_PROVIDER=llm_planner` 可明確啟用受控 LLM planner attempt，invalid / unavailable / timeout 時 fallback 到 deterministic planner；既有 tools 維持 read-only allowlist 並寫入 permission decision、tool tier、side-effect policy 與 human confirmation trace。v0.39.0 完成 deployment / observability / fine-tuning track release sync：K8s baseline manifests、opt-in Loki / Grafana local observability path、RAG / eval / worker JSONL trace logs 與 research-only synthetic data artifacts 已整理成可驗證交付物。v0.40.0 完成 Phase 40 JD evidence hardening release sync：Embedding / SFT research evidence、inference hardware benchmark evidence（KV cache / TOPS / NPU interpretation / provider skip reason）與 observability dashboard evidence（Loki / Grafana query examples / dashboard evidence skeleton）已整理成 validation-backed artifacts；這不是 production training、production inference autoscaling、production alerting 或 production guarantee。v0.42.0 完成 inference gateway / capacity planning release sync：provider routing / fallback metadata、streaming timeout guardrails、capacity planning report、workload profile 與 provider / hardware selection table 已形成 demo-safe inference ops evidence。v0.43.0 完成 AgentOps governance / secure tool runtime release sync：tool permission policy、approval fail-closed gate、Agent replay / eval artifact 與 forbidden boundary 已形成 demo-safe Agent governance evidence。v0.44.0 完成 Document Intelligence QA / human review loop release sync：field confidence / evidence view、human correction / golden labels、parser field accuracy smoke 與 tracked report 已形成 demo-safe 文件理解 QA evidence。v0.45.0 完成 final interview portfolio pack：JD evidence matrix、system design walkthrough、demo scenario pack 與 risk / tradeoff report 已形成可追問的面試證據入口。backend 仍不包含 SSO、OAuth、MFA、production login runtime、audit log pipeline、batch upload API、production deletion governance、restore workflow、Agent destructive tool、Qdrant cleanup API、production annotation platform、layout analysis、table reconstruction、model training、production autonomous Agent、任意 SQL、shell、filesystem command、external side-effect tool、production indexing pipeline、production autoscaling、multi-cluster deployment、managed secret integration 或 production training pipeline。
...
- 這個 demo auth runtime 不讀 Phase 32 PostgreSQL users table，也不是正式 JWT refresh rotation、SSO、OAuth、MFA、tenant isolation、project permission、Redis-backed production session、audit log 或 production RBAC。

Phase 32 Auth / RBAC schema foundation：
...
- `32-02` 新增 `users`、`organizations`、`projects`、`roles`、`memberships` 與 `project_memberships` PostgreSQL schema foundation。
- Migration command 使用 non-destructive `CREATE TABLE IF NOT EXISTS` / `CREATE INDEX IF NOT EXISTS`，demo seed rows 使用 upsert。
- Demo seed users 包含 Admin / Analyst / Viewer / disabled Viewer，password 只保存 PBKDF2 hash。
- 本階段不替換 Phase 28 demo auth runtime，也不新增 production login runtime。

Phase 32 backend permission guard：

- `DOCURAG_AUTH_MODE=formal` 會解析 signed bearer token 的 `sub`、`display_name`、`role`、`organization_id`、`project_ids` 與 active `project_id`。
- Document upload、OCR、parse、vector index、built-in eval 與 Agent run 需要 Analyst 或 Admin；Viewer 會收到 generic `403 forbidden`。
- Document list / detail / OCR result / fields / download、RAG query corpus、Agent search corpus 與 Agent run lookup 會依 project access filter / deny。
- `32-04` 已同步 frontend role surface 與 release version；`/auth/login` 在 formal mode 仍回傳尚未實作，Phase 32 不新增 SSO、OAuth、MFA、Redis-backed production session、audit log pipeline 或 production login runtime。

Phase 33 Redis runtime slice：

- `DOCURAG_REDIS_URL` 為空時，Redis 行為是 disabled，既有 demo API 不會受影響。
- 安裝 optional `backend[redis]` extra 並設定 `DOCURAG_REDIS_URL` 後，`/health` 會檢查 Redis，`/auth/login` 會 best-effort 寫入 token hash session cache，`/rag/query` 會使用短 TTL query cache 與簡單 rate limit。
- Redis client 未安裝或 Redis 連線不可用時，`/health` 會回報 `redis=unavailable`，login 與 RAG query 仍保留 fallback。
- 這不是 NATS worker pipeline、async queue、distributed lock runtime、production session rotation、OAuth、MFA 或 enterprise auth。

Phase 33 NATS worker skeleton：

- `DOCURAG_NATS_URL` 為空時，NATS helper 回 `disabled`；`DOCURAG_NATS_URL=memory://` 可用於本機 smoke。
- 安裝 optional `backend[nats]` extra 並設定真實 NATS URL 後，helper 可建立 publish / subscribe client；Docker image 需用 `DOCURAG_INSTALL_NATS=true` 才會安裝 client。
- `WorkerSkeleton` 會訂閱 `document.ocr.requested`、`document.parse.requested`、`document.index.requested` 與 `rag.eval.requested`，placeholder handler 只更新 task status，不執行實際 OCR / parser / indexing / eval。
- `GET /tasks` 與 `GET /tasks/{task_id}` 讀取 local JSON task records，狀態包含 `queued`、`running`、`succeeded`、`failed`、`retrying`、`cancelled`。
- 這不是 production async job queue、durable JetStream consumer、dead-letter dashboard、autoscaling 或 full observability stack。
...
- 預設 Agent planner 為 deterministic；`DOCURAG_AGENT_PLANNER_PROVIDER=llm_planner` 只會嘗試產生受 schema 驗證的 safe plan，invalid / unavailable / timeout 會 fallback 到 deterministic planner。
...
- Agent trace 會顯示 plan、tool calls、observation、final answer、citations、planner route、planner fallback、tool policy、permission decision、tool tier、side-effect policy 與 fallback count。
- 這不是 production autonomous Agent、任意 SQL、shell / file system command、worker、Redis、NATS、DB-backed tool console、destructive tool execution 或 production approval workflow。
...
Phase 44 Parser Field Accuracy Eval：

- `scripts/parser-field-accuracy-smoke.ps1` 會比較 `sample-data/eval/parser-golden-labels.json` 與 `sample-data/eval/parser-field-results.json`，輸出 field accuracy、document accuracy、missing field、wrong value、evidence mismatch、parser_source 與 confidence bucket。
- `sample-data/eval/parser-field-accuracy-report.json` 是可重跑的 demo-safe tracked report，用來呈現 parser output 和 human golden labels 的差距。
- 這不是 RAG retrieval eval、LLM-as-judge、model training、production analytics dashboard 或 automatic parser correction。
...
`DocumentStorage` 預設透過 `LocalJsonDocumentRepository` 保存 metadata 到 local JSON；設定 `DOCURAG_REPOSITORY_PROVIDER=postgresql` 並提供 `DOCURAG_DATABASE_URL` 時，會改用 opt-in `PostgresDocumentRepository` 保存 documents、chunks、parser fields、eval runs 與 Agent runs metadata。PostgreSQL mode 只做 non-destructive `CREATE TABLE IF NOT EXISTS` bootstrap 與 upsert writes，仍不代表 production DB operation、RBAC、worker queue 或 deployment hardening。
...
document metadata 也會保存 `processing_jobs` history 與 `latest_job` summary。同步 upload 會記錄 completed upload job；mock OCR 成功會記錄 completed OCR 與 local indexing job；provider failed 會記錄 failed OCR job。這些 job metadata 只是 document processing contract；Phase 33 worker task status 另存 `worker_tasks.json`，不取代 document metadata，也不代表實際 OCR / parser / indexing / eval 已改成 async execution。
...
- v0.45.0: Final Interview Portfolio Pack 已完成；backend package / app version、health test、Docker Compose `DOCURAG_VERSION`、`.env.example`、demo smoke expected version、README、README_DEV、backend README、frontend README、TODO、ROADMAP 與 ticket 已同步到 `0.45.0`。Phase 45 artifacts 覆蓋 JD evidence matrix、system design walkthrough、demo scenario pack 與 risk / tradeoff report；仍不包含新的 runtime feature、外部服務、production deployment、模型訓練、paid API、production guarantee 或 JD 100% 完成宣稱。Validation 已通過：backend full test、frontend build、demo smoke、Phase 45 evidence `rg` 與 `git diff --check`。
- v0.46.0: Admin Document Delete Flow 已完成；backend package / app version、health test、Docker Compose `DOCURAG_VERSION`、`.env.example`、demo smoke expected version、README、README_DEV、backend README、frontend README、TODO、ROADMAP 與 ticket 已同步到 `0.46.0`。`DELETE /documents/{document_id}` 會刪除文件 metadata、本機 upload artifact 與 PDF page image artifact，並維持 Admin / Analyst write guard、Viewer forbidden。Validation 已通過：focused backend document / auth / health tests、frontend build、Phase 46 keyword `rg` 與 `git diff --check`。仍不包含 Agent destructive tool、Qdrant cleanup API、restore workflow、production approval workflow、audit storage、worker 清理流程、migration 或 deployment 變更。
- v0.42.0: Inference Gateway / Capacity Planning 已完成；backend package / app version、health test、Docker Compose `DOCURAG_VERSION`、`.env.example`、demo smoke expected version、README、README_DEV、backend README、frontend README、TODO、ROADMAP 與 ticket 已同步到 `0.42.0`。Phase 42 artifacts 覆蓋 inference gateway contract、provider routing / fallback metadata、streaming timeout guardrails 與 capacity planning report；仍不包含 production autoscaling、多 GPU serving、paid API key、production secret、SLA、production metrics service 或 autoscaling controller。Validation 已通過：backend full test、frontend build、inference benchmark smoke skipped report、Phase 42 keyword `rg` 與 `git diff --check`。
- v0.43.0: AgentOps Governance / Secure Tool Runtime 已完成；backend package / app version、health test、Docker Compose `DOCURAG_VERSION`、`.env.example`、demo smoke expected version、README、README_DEV、backend README、frontend README、TODO、ROADMAP 與 ticket 已同步到 `0.43.0`。Phase 43 artifacts 覆蓋 Agent governance contract、tool permission policy metadata、approval fail-closed gate、Agent replay / eval sample 與 report；仍不包含 production autonomous Agent、任意 SQL、shell、filesystem command、destructive tool、external side-effect tool、production approval workflow 或 production audit storage。Validation 已通過：backend full test `269 passed, 1 warning`（pytest cache permission warning）、frontend build、Agent replay smoke、Phase 43 keyword `rg` 與 `git diff --check`。
- v0.44.0: Document Intelligence QA / Human Review Loop 已完成；backend package / app version、health test、Docker Compose `DOCURAG_VERSION`、`.env.example`、README、README_DEV、backend README、frontend README、TODO、ROADMAP 與 ticket 已同步到 `0.44.0`。Phase 44 artifacts 覆蓋 field confidence / evidence view、human correction / golden labels、parser field accuracy smoke 與 tracked report；仍不包含 production annotation platform、layout analysis、table reconstruction、model training 或 production OCR accuracy tuning。Validation 已通過：backend full test `270 passed, 1 warning`（pytest cache permission warning）、frontend build、parser field accuracy smoke（field accuracy `0.6`、sample count `5`、missing / wrong / evidence mismatch 各 `1`）、Phase 44 keyword `rg` 與 `git diff --check`。
- v0.41.0: RAG Quality Regression / DatasetOps 已完成；backend package / app version、health test、Docker Compose `DOCURAG_VERSION`、`.env.example`、demo smoke expected version、README、README_DEV、backend README、frontend README、TODO、ROADMAP 與 ticket 已同步到 `0.41.0`。Phase 41 artifacts 覆蓋 golden dataset metadata、retrieval regression CI report 與 chunking / indexing ablation report；仍不包含 production eval dashboard、LLM-as-judge、DB eval history、排程任務或外部 monitoring。Validation 已通過：backend full test、frontend build、retrieval regression report smoke、Phase 41 keyword `rg` 與 `git diff --check`。
...
- v0.31.0: PostgreSQL / Schema / Repository Foundation 已完成；新增 opt-in PostgreSQL metadata repository、local JSON fallback adapter、local JSON import command、backend repository tests 與 release sync；仍不包含正式 RBAC、worker pipeline 或 production deployment。
- v0.32.0: Formal Auth / RBAC / Tenant Boundary 已完成；新增 formal Auth / RBAC schema foundation、signed bearer permission guard、project access filtering、Viewer forbidden、frontend role surface 與 release sync；仍不包含 SSO、OAuth、MFA、Redis session、worker、deployment hardening 或 production login runtime。
...
- v0.34.0: Production OCR / Scanned PDF Pipeline 已完成；backend package / app version、health test、Docker Compose `DOCURAG_VERSION`、README、README_DEV、backend README、frontend README、TODO、ROADMAP 與 ticket 已同步。Validation 已通過：backend full test `233 passed`、scanned PDF demo smoke `4 passed`、frontend build、Browser desktop / mobile surface check、ticket `rg` 與 `git diff --check`。仍不包含 layout analysis、table reconstruction、human correction workflow、production OCR benchmark 或 production GPU scheduling。
- v0.35.0: RAG Indexing Quality Hardening 已完成；backend package / app version、health test、Docker Compose `DOCURAG_VERSION`、README、README_DEV、backend README、frontend README、TODO、ROADMAP 與 ticket 已同步。Validation 已通過：backend full test `240 passed`、frontend build、indexing quality smoke `7 passed`、ticket `rg` 與 `git diff --check`。仍不包含 production eval dashboard、LLM-as-judge、rerank tuning 或 production indexing worker。
- v0.36.0: Eval Dashboard / Rerank Analysis 已完成；backend package / app version、health test、Docker Compose `DOCURAG_VERSION`、README、README_DEV、backend README、frontend README、TODO、ROADMAP 與 ticket 已同步。Validation 已通過：backend full test `246 passed`、frontend build、eval dashboard smoke `7 passed, 27 deselected`、Chrome GUI DevTools desktop / mobile screenshot check、ticket `rg` 與 `git diff --check`。仍不包含 LLM-as-judge、answer faithfulness、citation quality scoring 或 production monitoring trend。
- v0.37.0: Inference Ops / vLLM Serving 已完成；backend package / app version、health test、Docker Compose `DOCURAG_VERSION`、README、README_DEV、backend README、frontend README、TODO、ROADMAP 與 ticket 已同步。Validation 已通過：backend full test `251 passed`（1 pytest cache warning）、frontend build、baseline demo smoke、inference benchmark smoke（本機 vLLM endpoint unavailable 時產出 `status=skipped` report）、ticket `rg` 與 `git diff --check`。仍不包含 production inference gateway、multi-GPU serving、K8s autoscaling、model registry、OpenAI billing / secret vault、RAG ranking 變更、VLM parser schema 變更或 Agent planner。
- v0.38.0: Agent Runtime Hardening 已完成；backend package / app version、health test、Docker Compose `DOCURAG_VERSION`、README、README_DEV、backend README、frontend README、TODO、ROADMAP 與 ticket 已同步。Validation 已通過：backend full test、frontend build、Agent runtime smoke、Browser Agent trace desktop / mobile、ticket `rg` 與 `git diff --check`。仍不包含 production autonomous Agent、任意 SQL、shell、filesystem command、destructive tool、external browser control 或 production approval workflow。
- v0.39.0: Deployment / Observability / Fine-tuning Track 已完成；backend package / app version、health test、Docker Compose `DOCURAG_VERSION`、`.env.example`、K8s sample image tag、README、README_DEV、backend README、frontend README、TODO、ROADMAP 與 ticket 已同步。Validation 已通過：backend full test `260 passed, 1 warning`（pytest cache permission warning）、frontend build、baseline demo smoke（health `0.39.0`，本機 Qdrant unavailable 時 keyword fallback 符合預期）、K8s offline YAML lint `15 documents`、observability smoke `5 passed, 1 warning`、Docker Compose observability profile config（Docker config permission warning，但 config 解析成功）、research artifact `rg`、JSONL parse sanity check、release `rg` 與 `git diff --check`。`kubectl apply --dry-run=client --validate=false -f .\infra\k8s` 已嘗試，但本機無 Kubernetes API context，kubectl 在 `localhost:8080` discovery 階段失敗。仍不包含 production autoscaling、multi-cluster deployment、managed secret integration、production alerting / incident workflow 或 production training pipeline。
- v0.40.0: JD Evidence Hardening 已完成；backend package / app version、health test、Docker Compose `DOCURAG_VERSION`、`.env.example`、K8s sample image tag、demo smoke expected version、README、README_DEV、backend README、frontend README、TODO、ROADMAP 與 ticket 已同步到 `0.40.0`。Phase 40 artifacts 覆蓋 Embedding / SFT evidence、inference hardware benchmark evidence（KV cache / TOPS / provider skip reason）與 observability dashboard evidence；仍不包含 production training、production inference autoscaling、production alerting / incident workflow 或 production guarantee。Validation 已通過：backend full test `260 passed, 1 warning`（pytest cache permission warning）、frontend build、baseline demo smoke（health `0.40.0`）、Phase 40 evidence `rg`、release `rg` 與 `git diff --check`（只有 LF/CRLF 提示）。
