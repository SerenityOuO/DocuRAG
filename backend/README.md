...
- 這個 demo auth runtime 不讀 Phase 32 PostgreSQL users table，也不是正式 JWT refresh rotation、SSO、OAuth、MFA、tenant isolation、project permission、Redis session、audit log 或 production RBAC。
...
- `32-04` 已同步 frontend role surface 與 release version；`/auth/login` 在 formal mode 仍回傳尚未實作，Phase 32 不新增 SSO、OAuth、MFA、Redis session、audit log pipeline 或 production login runtime。
