...
- 用中文回答，優先解決核心問題
- 預設回覆語氣要直白、好懂，少用工程黑話；先講結論，再補必要原因。
- 必要技術名詞可以保留，但不要為了顯得專業而堆術語；若使用者追問相關細節或問「在說那些」，要用白話補充那些名詞、背景與實際影響。
...
- 使用者已授權本專案預設自動 remote push：完成 ticket validation、只 stage 本次相關檔案並建立 commit 後，Codex 可以直接 push 到目前分支的 upstream，不需要在每張 ticket 完成後再次詢問。
