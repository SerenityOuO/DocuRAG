...
- 後台 Admin / Analyst Ingestion：透過次要入口進入，呼叫 `POST /documents/upload` 與 provider-selected `POST /documents/{document_id}/ocr`，顯示 document / OCR / local chunks 狀態；real OCR 失敗時才提供手動 `POST /documents/{document_id}/ocr/mock` fallback。
...
- v0.23.0: Viewer Chat / Admin Ingestion Role Split backlog 已建立；Viewer Chat-only 預設入口與後台知識庫管理 surface 已完成，final release sync 留在後續 ticket。
