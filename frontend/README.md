...
- 文件上傳：呼叫 `POST /documents/upload`，再呼叫 provider-selected `POST /documents/{document_id}/ocr` 完成 real GPU OCR-first backend OCR flow；若 real OCR 失敗，保留已上傳文件並提供手動 `POST /documents/{document_id}/ocr/mock` fallback。
