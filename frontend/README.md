...
目前 frontend package 與 fallback label 已同步到 `0.40.0`。Phase 40 的 JD evidence hardening artifacts（Embedding / SFT、inference hardware benchmark、observability dashboard evidence）主要由 backend、infra、docs 與 smoke scripts 驗證；frontend 不新增 production inference dashboard、production observability dashboard、production alerting UI 或 production training UI。
