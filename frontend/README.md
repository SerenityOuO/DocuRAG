...
- 後台 Admin / Analyst Ingestion：呼叫 `POST /documents/upload`，再呼叫 provider-selected `POST /documents/{document_id}/ocr` 完成 real GPU OCR-first backend OCR flow；若 real OCR 失敗，保留已上傳文件並提供手動 `POST /documents/{document_id}/ocr/mock` fallback。這是知識庫管理入口，不是 Viewer 直接對圖片聊天。
...
- v0.23.0: Viewer Chat / Admin Ingestion Role Split backlog 已建立；後續 ticket 會把 Viewer Chat 與後台知識庫管理 surface 拆開。
