...
- 後台 Admin / Analyst Ingestion：透過次要入口進入，呼叫 `POST /documents/upload` 與 provider-selected `POST /documents/{document_id}/ocr`，顯示 document / OCR / parser / local chunks 狀態；real OCR 失敗時才提供手動 `POST /documents/{document_id}/ocr/mock` fallback。
