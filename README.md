...
- v0.26.0 backlog 已新增 Real VLM Parser Provider Spike：下一步會以 VLM-first provider、demo-safe image input resolver、`vlm_invoice` adapter 與 parser source comparison 承接 structured fields；`deterministic_invoice` 只作 fallback / debug override，Agent contract 不變，只透過 `get_document_fields` 消費 parser result。
...
4. 在後台觸發 parser 後執行 Agent trace demo，說明 deterministic planner 如何用 `get_document_fields`、`search_documents`、`summarize_invoice_fields` 形成 source-backed final answer。
...
7. 補充 optional paths：`vector`、`vector_rerank`、`hybrid` 與 `hybrid_rerank` 都是 explicit eval / demo path；`hybrid_rerank` 不接 default `/rag/query` 或 frontend chat route。
