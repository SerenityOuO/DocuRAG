...
目前主線已完成 local document workflow、provider-selected PaddleOCR OCR flow、mock OCR override、local keyword RAG baseline、frontend demo UI，以及可重跑的本機與 Docker backend 環境。這仍是受控 MVP，不是 production OCR / RAG 平台。
...
- Python 3.12 backend runtime；real OCR dependency 收斂在 `backend[real-ocr]` optional extra。
- Dockerfile / Docker Compose backend runtime，real OCR dependency 可透過 build arg 開啟。
...
建置包含 real OCR dependency 的 backend image：
...
3. 完成後更新對應 checklist，並執行 ticket 指定 validation。
4. 嚴格避免把後續 OCR、RAG、infra、auth 或 database scope 提前塞進當前 ticket。
...
- Phase 08: PaddleOCR Runtime Stabilization 已完成；Python 3.12、PaddleOCR 2.10.0、PaddlePaddle 3.0.0 real OCR sample flow 已驗證，provider-selected OCR 預設走 PaddleOCR，mock flow 需透過 `/ocr/mock` 或 `DOCURAG_OCR_PROVIDER=mock` 明確 override。
