...
目前文件中的最新已完成展示版本是 `v0.27.1`。Phase 27 的 vector source contract 已補齊；Phase 28 已有 backlog 規劃，但尚未標記完成。
...
- Vector source contract：目前 vector indexing 主要吃 `ocr_image` chunks；`.txt` direct chunks、`pdf_text` 與 `pdf_scanned_pending_ocr` 已拆成後續小票邊界。
...
3. 切到 Admin / Analyst ingestion，展示文件上傳、OCR 文字層、VLM parser structured fields 與欄位 evidence 狀態。
...
- `.txt` direct text ingestion、text-native PDF extraction 與 scanned PDF rendering / OCR pipeline；目前 Qdrant demo source contract 已定義，但 runtime 仍主要索引 OCR chunks。
