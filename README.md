...
目前文件中的最新已完成展示版本是 `v0.28.0`。Phase 28 已補齊 Document Source Router contract、`.txt` direct ingestion、text-native PDF ingestion 與 demo login mode。
...
5. 展示 Agent trace，說明 deterministic planner 如何用 allowlisted tools 讀取欄位、搜尋文件並產生 source-backed answer。
6. 展示 retrieval eval smoke 結果，說明 Hit Rate@K、MRR@K、Recall@K 與 fallback count 如何幫助分析 RAG 品質。
