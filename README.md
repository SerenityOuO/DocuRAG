...
- v0.26.0 backlog 已新增 Real VLM Parser Provider Spike：下一步會以 disabled-by-default VLM provider、demo-safe image input resolver、`vlm_invoice` adapter 與 parser source comparison 承接 structured fields；Agent contract 不變，只透過 `get_document_fields` 消費 parser result。
...
- Production-grade VLM parser、default-on vision runtime、PDF rendering、多頁 parser pipeline、人工修正欄位版本紀錄或表格完整重建。
