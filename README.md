...
- Vue 3 + Vite frontend 只暴露客服問答與文件上傳；上傳會觸發 backend provider-selected OCR，OCR detail、chunking、indexing、document list、raw JSON 與 detailed trace table 都留在 backend API / CLI / smoke scripts。
...
3. 說明 frontend 只負責問問題與上傳文件；文件上傳後預設走 backend provider-selected GPU OCR，OCR detail、chunking、indexing、metadata 與 detailed trace 都在 backend / CLI 層處理。
...
| 文件上傳 | Analyst / Admin 可以把文件送到後端 GPU OCR-first 知識庫流程；OCR、chunking 與 indexing 不在 frontend 展開。 |
