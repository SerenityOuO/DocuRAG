...
- Vue demo UI：Phase 23 產品邊界拆成 Viewer Chat 與 Admin / Analyst Ingestion。Viewer 只查詢已建立知識庫；文件上傳、OCR result、structured fields、Agent trace、chunks、document list、retrieved chunks、trace metadata 與 eval metrics 留在後台 surface、backend API、CLI 或 smoke scripts。Phase 28 source router 目前已支援 `.txt` direct import 與 text-native PDF extraction，並保留 image OCR 與 scanned PDF pending OCR 的分流邊界；demo auth mode 可先登入 Admin / Analyst / Viewer，Viewer 看不到 ingestion controls。
...
04:30 - 05:40  Trigger VLM-first parser and show invoice structured fields, parser source, OCR evidence mapping and fallback trace.
05:40 - 07:00  Run Agent trace: deterministic plan -> allowlisted tools -> observations -> source-backed final answer.
07:00 - 07:45  Explain OCR -> structured fields -> Agent tool-use -> local chunks -> RAG citation flow without calling it production indexing.
07:45 - 08:40  Open trace panel and explain retrieval source, candidate rows and fallback states.
08:40 - 09:30  Explain Phase 28 source router: image_ocr, text_upload, pdf_text and pdf_scanned_pending_ocr.
09:30 - 10:00  Explain strategy controls: default hybrid_rerank, keyword fallback and optional eval comparison.
```
