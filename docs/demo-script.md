...
目前可展示的是受控 single-user / local-demo MVP：v0.17.0 已完成 frontend trace visibility，v0.18.0 是 `hybrid_rerank` planning-only，v0.19.0 已把 `hybrid_rerank` 落地成 optional eval runner strategy，v0.20.0 完成 interview packaging，v0.21.0 將 upload + OCR demo path 改為 real GPU OCR-first，v0.23.0 完成 Viewer Chat / Admin Ingestion role split，v0.24.0 補上 deterministic invoice parser fallback、parse / fields API 與 frontend structured fields surface，v0.25.0 補上 deterministic Agent tool-use trace，v0.26.0 補上 VLM-first parser provider spike、fake / stub success validation 與 provider unavailable fallback，把 structured fields、document search 與 deterministic invoice summary 串成 source-backed final answer。這不是 production-ready 平台，而是可說清楚取捨的面試展示切片。
...
04:00 - 05:15  Trigger VLM-first parser and show invoice structured fields, parser source and fallback trace.
...
- Phase 26 parser 是 VLM-first provider spike：`vlm_invoice` success path 與 `deterministic_invoice` fallback 共用同一個 `DocumentFields` schema，可展示 parser source、confidence 與 fallback trace。
