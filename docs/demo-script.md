...
目前可展示的是受控 single-user / local-demo MVP：v0.26.0 補上 VLM-first parser provider spike、fake / stub success validation 與 provider unavailable fallback；v0.27.0 把已完成的進階路徑改成 aggressive defaults，預設用 `hybrid_rerank` 查詢、Ollama embedding、FastEmbed rerank adapter，後台 OCR 後 best-effort 執行 parser 與 Qdrant vector indexing。這不是 production-ready 平台，而是可說清楚取捨的面試展示切片。
...
01:00 - 03:00  Admin / Analyst Ingestion: upload a sample image and let backend run provider-selected GPU OCR, VLM-first parser and best-effort vector indexing.
