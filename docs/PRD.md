...
目前 MVP 不追求一次完成所有 AI 與 infra 元件，而是用 single-user / local-demo 邊界逐步交付可展示切片。到 v0.27.0 為止，主線已具備文件上傳、provider-selected OCR、VLM-first parser、Agent tool-use trace、citation trace、retrieval eval runner、frontend trace visibility，以及 default `hybrid_rerank` RAG / Agent search with fallback。這仍是 demo-first runtime，不把 production DB、worker、auth 或 deployment hardening 提前塞進目前 MVP。
