...
- TODO 包含 Phase 00 到 v0.36.0 Eval Dashboard / Rerank Analysis checklist。
...
- v0.35.0 RAG Indexing Quality Hardening 只做 chunking strategy runtime、Qdrant payload filter / index、project reindex、stale cleanup 與 release sync；不新增 production eval dashboard、LLM-as-judge、rerank tuning 或 production indexing worker。
- v0.36.0 Eval Dashboard / Rerank Analysis 只做 eval dataset management、strategy comparison、rerank before / after analysis、trace metadata coverage、failure / fallback cases 與 release sync；不新增 LLM-as-judge、answer faithfulness、citation quality scoring、production monitoring trend、production indexing worker 或 OCR eval。
- v0.37.0 Inference Ops / vLLM Serving 只做 OpenAI-compatible provider boundary、vLLM local / Docker serving docs、inference benchmark smoke、latency / token / throughput / KV cache / GPU memory estimate 與 release sync；不新增 production inference gateway、multi-GPU serving、autoscaling、model registry、secret vault、RAG ranking 變更、VLM parser schema 變更或 Agent planner。
...
- 已完成 Phase 31 `v0.31.0` release sync、Phase 32 `v0.32.0` Formal Auth / RBAC / Tenant Boundary release sync、Phase 33 `v0.33.0` Redis + NATS Worker Pipeline release sync、Phase 34 `v0.34.0` Production OCR / Scanned PDF Pipeline release sync、Phase 35 `v0.35.0` RAG Indexing Quality Hardening release sync、Phase 36 `v0.36.0` Eval Dashboard / Rerank Analysis release sync、Phase 37 `v0.37.0` Inference Ops / vLLM Serving release sync、Phase 38 `v0.38.0` Agent Runtime Hardening release sync 與 Phase 39 `v0.39.0` Deployment / Observability / Fine-tuning Track release sync。`31-02` PostgreSQL boundary / migration policy、`31-03` DB schema contract、`31-04` repository adapter / migration path、`31-05` release sync、`32-01` 到 `32-04`、Phase 33 `33-01` 到 `33-04`、Phase 34 `34-01` 到 `34-04`、Phase 35 `35-01` 到 `35-04`、Phase 36 `36-01` 到 `36-04`、Phase 37 `37-01` 到 `37-04`、Phase 38 `38-01` 到 `38-04` 與 Phase 39 `39-01` 到 `39-05` 均已完成。
...
- Completed `v0.33.0` release sync across backend package / app version, frontend package / lock / fallback version, health test, Docker Compose `DOCURAG_VERSION`, `.env.example`, README, README_DEV, backend README, frontend README, TODO, ROADMAP and the ticket.
- Added `scripts/worker-demo-smoke.ps1` to verify Redis session cache / query cache / rate limit with a fake client, NATS `memory://` publish / consume with the worker skeleton, and `/tasks` task status readback.
- Release boundary remains explicit: Phase 33 is a demo-safe async architecture milestone, not production autoscaling, durable JetStream consumer, full observability, vLLM, OpenAI API, fine-tuning or production OCR / parser / indexing / eval worker execution.
- Validation passed: backend test script (`227 passed`, 1 pytest cache warning), frontend build, `scripts/worker-demo-smoke.ps1` (health version `0.33.0`, Redis fake-client path `ok`, NATS memory worker and task status `succeeded`), ticket `rg` and `git diff --check`.
...
- Page image records, OCR blocks, page-level status, retry state and failure reasons are defined for `34-02` and `34-03` runtime tickets.
...
34-02 PDF Rendering Page Image Pipeline Status:
...
36-04 Eval Dashboard Phase 36 Release Sync Status：

- Completed `v0.36.0` release sync across backend package / app version, frontend package / lock / fallback version, health test, Docker Compose `DOCURAG_VERSION`, `.env.example`, README, README_DEV, backend README, frontend README, TODO, ROADMAP and the ticket.
- Eval dashboard smoke covers eval dataset setup, strategy comparison, failure / fallback cases and rerank analysis visibility.
- Release boundary remains explicit: this is an eval dashboard / rerank analysis release, not LLM-as-judge, answer faithfulness, citation quality scoring or production monitoring trend.
- Validation passed: backend full tests `246 passed`; frontend build; `scripts/eval-dashboard-smoke.ps1` (`7 passed, 27 deselected`); Chrome GUI DevTools desktop / mobile screenshot check; ticket `rg` and `git diff --check`.
...
- `38-04` 已完成 Phase 38 release sync。backend package / app version、frontend package / lock / fallback version、health test、Docker Compose `DOCURAG_VERSION`、`.env.example`、README、README_DEV、backend README、frontend README、TODO、ROADMAP 與 ticket 已同步到 `0.38.0` / `v0.38.0`。
- `scripts/agent-runtime-smoke.ps1` 已納入 Agent runtime validation，可檢查 planner fallback、tool permission guard、Viewer forbidden path、saved run lookup 與 trace 欄位；Browser desktop / mobile 也確認 Agent trace permission fields 正常顯示且無 horizontal overflow。
- Validation passed: backend full test `255 passed, 1 warning`（pytest cache warning）；frontend build；Agent runtime smoke（health `0.38.0`、planner fallback `llm_planner_timeout`、Viewer 403、permission trace OK）；Browser Agent trace desktop / mobile；ticket `rg`；`git diff --check`（僅 Windows LF/CRLF 提示）。
- Release boundary 保持明確：Phase 38 仍不允許 arbitrary autonomous execution、任意 SQL、shell、filesystem command、destructive tools、external browser control、production approval workflow 或 production audit dashboard。
- Release Impact：Version bump required: yes。Phase 38 is released as `v0.38.0`。
...
- Release Impact：Version bump required: no。`39-02` 是 deployment artifact baseline；sample image tag 已由 `39-05` release sync 更新到 `0.39.0`。
...
- Release Impact：Version bump required: no。`39-03` 是 observability runtime / docs slice；版本已由 `39-05` release sync 統一更新到 `v0.39.0`。不新增 production alerting、SLO、distributed tracing、APM vendor integration、long-term storage 或 incident workflow。
...
- Release Impact：Version bump required: no。`39-04` 只新增 research artifacts，不執行 training、不下載大型模型、不新增 dependency、不接 production runtime；版本已由 `39-05` release sync 統一更新到 `v0.39.0`。
- `39-05` 已完成 Phase 39 `v0.39.0` release sync。backend package / app version、frontend package / lock / fallback version、health test、Docker Compose `DOCURAG_VERSION`、`.env.example`、K8s sample image tag、README、README_DEV、backend README、frontend README、TODO、ROADMAP、demo smoke expected version 與 ticket 已同步。
- Validation passed: backend full test `260 passed, 1 warning`（pytest cache permission warning）；frontend build；baseline demo smoke（health `0.39.0`，本機 Qdrant unavailable 時 keyword fallback 符合預期）；K8s offline YAML lint `15 documents`；observability smoke `5 passed, 1 warning`；Docker Compose observability profile config（Docker config permission warning，但 config 解析成功）；research artifact `rg`；JSONL parse sanity check；release `rg`；`git diff --check`。`kubectl apply --dry-run=client --validate=false -f .\infra\k8s` 已嘗試，但本機無 Kubernetes API context，kubectl 在 `localhost:8080` discovery 階段失敗。
- Release Impact：Version bump required: yes。Phase 39 is released as `v0.39.0`；不新增 production autoscaling、multi-cluster deployment、managed secret integration、production alerting / incident workflow 或 production training pipeline。
...
44-01 Document Intelligence QA Contract Status：
...
- [x] `tasks/phase-45-production-readiness-portfolio-pack/45-05-phase-45-final-release-sync.md`
...
45-01 JD Evidence Matrix Status：
...
45-05 Phase 45 Final Release Sync Status：
...
20-07 Frontend Zh-TW Copy Polish Status：

- 新增 frontend Zh-TW copy polish ticket，目標是把既有 demo 網頁中可控的可見英文文案改成繁體中文，讓面試展示語言更一致。
...
Goal：承接 Phase 24 parser schema 與 Phase 25 Agent tool-use，新增 VLM-first parser provider spike。Phase 26 的核心不是讓 Agent 直接呼叫 VLM，而是讓 VLM parser 預設優先產生 structured fields，VLM 不可用或回傳不合法時才 fallback 到 deterministic parser，讓既有 `get_document_fields` tool 可以讀取保存後的結果。
...
- VLM-first `vlm_invoice` adapter 可把 fake / configured provider output 正規化成既有 `DocumentFields` / `ParserResult`。
...
- `vlm_invoice` 必須作為 Phase 26 parser 預設優先路徑；沒有可用 VLM provider、timeout 或 invalid response 時 demo 才回到 deterministic parser。
