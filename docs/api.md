...
### 43-04 Agent Replay and Eval Artifact

`43-04` adds a demo-safe Agent replay artifact and smoke report. Replay is inspection-only evidence: it reads saved run-shaped JSON, reconstructs plan steps, tool calls, observations, fallback reason and final answer source, then evaluates the artifact without executing tools or calling external services.

Replay artifact files:

| Path | Purpose |
|---|---|
| `sample-data/eval/agent-replay-sample.json` | Source Agent replay artifact with policy snapshot, plan steps, tool calls, observations, citations, fallback reason and final answer. |
| `sample-data/eval/agent-replay-report.json` | Deterministic report generated by `scripts/agent-replay-smoke.ps1`. |
| `scripts/agent-replay-smoke.ps1` | Smoke script for replay validation. Default output goes to `.tmp/agent-replay-report.json`; release evidence can pass `-ReportPath`. |

Report dimensions are `tool correctness`, `permission compliance`, `evidence coverage`, `fallback reason` and `groundedness`. `groundedness` is a deterministic evidence note based on citations and tool observations, not LLM-as-judge or answer faithfulness scoring. Replay mode must remain `inspection_only_no_tool_execution`; destructive, external side-effect, arbitrary SQL, shell and filesystem command tools remain out of scope.
