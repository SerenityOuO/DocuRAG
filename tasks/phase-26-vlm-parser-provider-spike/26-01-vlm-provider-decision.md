...
固定 Phase 26 Real VLM Parser Provider Spike 的產品與技術邊界，決定 VLM-first parser default、provider contract、env 設定、輸入 / 輸出格式、fallback policy 與 demo guardrails；本 ticket 只做規格與文件，不實作 runtime。

## Scope

- 定義 Phase 26 的目標：在 Phase 24 `DocumentFields` / `ParserResult` schema 上把 `vlm_invoice` 設為 parser 預設優先路徑。
...
- 定義 fallback policy：預設先嘗試 `vlm_invoice`；VLM provider unavailable、timeout、unsupported file、invalid JSON 或 confidence too low 時，回到 Phase 24 deterministic parser，不覆蓋既有 OCR / parser 狀態。
- 定義 deterministic parser 角色：`deterministic_invoice` 不再是 Phase 26 後的預設路徑，只能作為 VLM fallback 或明確 debug / validation override。
...
- 不新增 production-grade VLM parser、OpenAI SDK、function calling、streaming 或新外部依賴；Phase 26 的 default-on 僅代表 demo parser path 預設 VLM-first。
...
- [ ] 文件明確說明 `vlm_invoice` 是 Phase 26 parser 預設優先路徑，`deterministic_invoice` 只作 fallback / debug override。
