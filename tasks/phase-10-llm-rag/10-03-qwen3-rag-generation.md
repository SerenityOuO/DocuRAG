# 10-03 Qwen3.5 RAG Generation

## Goal

在既有 keyword retrieval 與 citations contract 上加入 `qwen3.5:4b` LLM answer generation，讓 `/rag/query` 可以用 retrieved chunks 產生自然語言回答並保留 citation trace。

## Scope

- 在 `RagProvider` 邊界內新增 LLM generation path。
- 使用 `qwen3.5:4b` 作為第一版 generation model。
...
- [ ] `/rag/query` 可在 LLM enabled 時使用 retrieved chunks 與 `qwen3.5:4b` 產生回答。
- [ ] citations 與 retrieved chunks 仍與原 chunk metadata 對齊。
- [ ] LLM unavailable 時不產生誤導性回答。
- [ ] trace metadata 記錄 `provider=ollama` 與 `model=qwen3.5:4b`。
- [ ] 測試覆蓋 prompt、成功生成、失敗處理與 citation trace。

## Validation

- `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\test-backend.ps1`
- 以本機 Ollama `qwen3.5:4b` 或 mock LLM client 執行 RAG query smoke。
