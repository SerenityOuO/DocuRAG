# 10-02 Ollama Qwen3.5 LLM Client

## Goal

新增最小 Ollama LLM client，讓 backend 可以呼叫本機 `qwen3.5:4b`，但不立即改變既有 `/rag/query` 預設行為。

## Scope

- 新增 LLM provider interface 與 Ollama HTTP client。
- 以 `DOCURAG_LLM_PROVIDER=ollama` 啟用 provider。
- 使用 `DOCURAG_LLM_BASE_URL` 與 `DOCURAG_LLM_MODEL=qwen3.5:4b` 設定本機服務。
...
- [ ] 設定 `DOCURAG_LLM_PROVIDER=ollama` 與 `DOCURAG_LLM_MODEL=qwen3.5:4b` 時，client 可送出本機生成 request。
