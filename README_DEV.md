...
後續 roadmap 已新增 Phase 31 到 Phase 39，並建立對應 future ticket backlog；目前這些 tickets 尚未實作，不是已完成 release。建議順序為：`v0.31.0` PostgreSQL / schema / repository foundation、`v0.32.0` formal Auth / RBAC / tenant boundary、`v0.33.0` Redis + NATS worker pipeline、`v0.34.0` production OCR / scanned PDF pipeline、`v0.35.0` RAG indexing quality hardening、`v0.36.0` eval dashboard / rerank analysis、`v0.37.0` inference ops / vLLM serving、`v0.38.0` Agent runtime hardening、`v0.39.0` deployment / observability / fine-tuning track。每個 Phase 都已拆成 contract / runtime / validation / release sync 類型的小票；後續仍必須逐張 ticket 執行，完成 release sync ticket 前不更新版本號。
