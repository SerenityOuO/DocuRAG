...
- Production autonomous Agent、可執行 LLM planner、任意 SQL / tool execution 或 destructive tools。Phase 38 `38-01` 只定義 Agent permission model contract，尚未新增 runtime。
...
後續 roadmap 已新增 Phase 31 到 Phase 40，並建立對應 future ticket backlog；目前 Phase 31 已在 `v0.31.0` 完成 PostgreSQL / schema / repository foundation release sync，Phase 32 已在 `v0.32.0` 完成 formal Auth / RBAC / tenant boundary release sync，Phase 33 已在 `v0.33.0` 完成 Redis + NATS worker demo milestone release sync，Phase 34 已在 `v0.34.0` 完成 scanned PDF OCR baseline release sync，Phase 35 已在 `v0.35.0` 完成 RAG indexing quality hardening release sync，Phase 36 已在 `v0.36.0` 完成 eval dashboard / rerank analysis release sync，Phase 37 已在 `v0.37.0` 完成 inference ops / vLLM serving release sync，Phase 38 `38-01` 已完成 Agent runtime permission contract。建議後續順序為：Phase 38 剩餘 runtime tickets 與 `v0.38.0` release sync、`v0.39.0` deployment / observability / fine-tuning track、`v0.40.0` JD evidence hardening。每個 Phase 都已拆成 contract / runtime / validation / release sync 類型的小票；Phase 40 則聚焦 Embedding / SFT evidence、inference hardware benchmark evidence 與 observability dashboard evidence。後續仍必須逐張 ticket 執行，不得提前實作下一個 Phase 的 runtime。
