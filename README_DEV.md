...
v0.42.0 已完成 Phase 42 inference gateway / capacity planning release sync：provider routing / fallback metadata、timeout guardrails、non-streaming demo boundary、capacity planning report 與模型 / 硬體選型表已整理成可驗證推論維運證據；這仍不是 production autoscaling、多 GPU serving、paid API key、production secret、SLA、production metrics service 或 autoscaling controller。
Phase 43 `43-01` 已完成 Agent governance contract：`docs/architecture.md` 與 `docs/api.md` 已固定 tool policy、risk score、approval states、audit event、replay event、trace completeness 與 forbidden destructive boundary；這只定義治理資料邊界，不改 deterministic Agent planner、不新增 runtime tool execution，也不開放任意 SQL、shell、filesystem command、production approval workflow 或 audit log pipeline。
Phase 43 `43-02` 已完成最小 tool permission policy runtime 補強：既有 allowlisted Agent tools 仍維持 read-only / Admin + Analyst / no-side-effect，trace 會補上 `risk_tier`、`risk_score`、`approval_required` 與 `approval_state`，Viewer path 仍在 tool execution 前以 generic permission reason 擋下；本 ticket 不新增 write / admin / destructive tool，也不新增外部 approval service。
Phase 43 `43-03` 已完成 human approval risk tier 的最小 fail-closed 行為：future high-risk policy 若 `approval_required=true` 且狀態不是 `approved`，會在 tool execution 前以 `approval_required`、`approval_rejected` 或 `approval_expired` 擋下並留下 trace；目前 allowlisted runtime tools 仍是 read-only / `not_required`，不新增 approval API、UI、外部 workflow 或 destructive tool。
Phase 43 `43-04` 已完成 Agent replay / eval artifact：新增 inspection-only replay sample、`scripts/agent-replay-smoke.ps1` 與 deterministic report，能檢查 tool correctness、permission compliance、evidence coverage、fallback reason 與 groundedness notes；本 ticket 不重跑 Agent tools、不新增 backend runtime、production audit storage、LLM-as-judge 或 destructive / external side-effect replay。
v0.43.0 已完成 Phase 43 AgentOps governance / secure tool runtime release sync：tool permission policy、approval fail-closed gate、Agent replay / eval artifact 與 forbidden boundary 已整理成可驗證 Agent 治理證據；這仍不是 production autonomous Agent、任意 SQL、shell、filesystem command、destructive tool、external side-effect tool、production approval workflow 或 production audit storage。
Phase 44 `44-01` 已完成 Document Intelligence QA contract：`docs/architecture.md` 與 `docs/api.md` 已固定 field confidence、evidence source、source page / bbox、review status、correction version、human correction、golden labels 與 parser field accuracy 指標邊界；這只定義文件理解 QA 資料語意，不改 OCR / parser default behavior、不新增 full annotation platform、production workflow、多人審核權限或外部 labeling tool。
Phase 44 `44-02` 已完成 Field Confidence and Evidence View：Admin / Analyst structured fields surface 會清楚顯示 field confidence、evidence linked / unmatched / unavailable、source text、source page、source bbox、parser source 與 fallback reason；Viewer Chat 仍維持 read-only 查詢主線，本 ticket 不新增人工修正寫入、golden labels、parser accuracy eval、OCR / VLM provider 或 full document image annotation UI。
Phase 44 `44-03` 已完成 Human Correction and Golden Labels：Admin / Analyst 可保存 demo-safe corrected value，backend 會以 append-only correction metadata 保存 reviewer、reason、version、timestamp 與 source parser value；Viewer 不能修改 correction。Golden labels export 會輸出 latest corrected fields，供 `44-04` parser field accuracy eval 使用；這不是 production annotation workflow、多人審核、外部 labeling service、model training writeback 或 destructive edit / delete flow。
Phase 44 `44-04` 已完成 Parser Field Accuracy Eval：新增 demo-safe parser field accuracy smoke、parser result fixture 與 tracked report，可比較 human golden labels 和 parser output，輸出 field accuracy、document accuracy、missing field、wrong value、evidence mismatch、parser_source 與 confidence bucket；這不是 RAG retrieval eval、LLM-as-judge、model training、production analytics dashboard 或 automatic parser correction。
v0.44.0 已完成 Phase 44 Document Intelligence QA / human review loop release sync：field confidence / evidence view、human correction / golden labels、parser field accuracy eval 與 validation report 已整理成可展示的文件理解 QA 證據；這仍不是 production annotation platform、layout analysis、table reconstruction、model training 或 production OCR accuracy tuning。
Phase 45 `45-01` 已完成 JD Evidence Matrix：新增 `docs/jd-evidence-matrix.md`，把 AI Core、Software / System Architecture、Inference & Ops 的 JD keyword 對到專案證據、demo command、file path、status、面試說法、honesty boundary 與 next action；這是 portfolio artifact，不新增 runtime、dependency、測試程式、截圖或 demo media，也不 bump version。
Phase 45 `45-02` 已完成 System Design Walkthrough：新增 `docs/system-design-walkthrough.md`，可用 10 分鐘說明 Viewer Chat、Admin Ingestion、OCR / VLM parser、RAG indexing / eval、AgentOps、Redis / NATS、PostgreSQL、inference gateway、observability、fallback 與 tradeoff；這是 portfolio documentation，不新增架構圖生成工具、runtime、infra config 或 deployment service。
Phase 45 `45-03` 已完成 Demo Scenario Pack：新增 `docs/demo-scenario-pack.md`，把 RAG quality、Document Intelligence QA、AgentOps governance 拆成可面試操作的主線，並補 Inference Gateway、Observability、K8s / deployment boundary optional path；每條路線都列前置條件、操作步驟、預期畫面 / 輸出與 fallback 解讀。本 ticket 不新增 demo runtime、sample secrets、外部 paid API 或影片媒體。
Phase 45 `45-04` 已完成 Risk Boundary and Tradeoff Report：新增 `docs/risk-boundary-and-tradeoff-report.md`，誠實整理 data isolation、production DB migration、worker durability、observability、model latency、OCR / VLM accuracy 與 Agent safety 的 mitigation、剩餘缺口、下一步與面試 tradeoff 回答；這是 portfolio risk report，不新增 runtime safety control、production incident workflow、SLO、pager 或 external vendor integration。
v0.45.0 已完成 Phase 45 final interview portfolio pack release sync：JD evidence matrix、system design walkthrough、demo scenario pack 與 risk / tradeoff report 已整理成最終面試作品包，backend / frontend / Docker Compose / health test / demo smoke expected version 已同步到 `0.45.0`；這仍不是 production guarantee、JD 100% 完成宣稱、外部服務、production deployment、模型訓練或 paid API。
v0.46.0 已完成 Phase 46 Admin Document Delete Flow：Admin / Analyst 可在後台資料匯入狀態刪除文件，backend 會移除文件 metadata、本機 upload artifact 與 PDF page image artifact；Viewer 仍被 role guard 擋下。這不新增 Agent destructive tool、Qdrant vector cleanup API、restore / recycle bin、production approval workflow、audit storage、worker 清理流程、migration 或 deployment 變更。

Post-v0.45.0 focused hardening：`tasks/admin-ingestion-ui-filename-copy-hardening.md` 修正後台匯入狀態的中文檔名與欄位修正文案。Upload filename sanitization 現在會保留 Unicode 檔名，避免 `周峻緯履歷.pdf` 被保存成 `pdf`；Admin / Analyst structured fields surface 將 `corrected value`、`reviewer reason` 與 `golden label` 改成「修正後欄位值」、「修正原因」與「標準答案」。Validation 已通過 backend document tests 與 frontend build。本 follow-up 不新增 document delete API、文件刪除按鈕或 destructive edit / delete flow。

Phase 41-46 已完成 JD completion / portfolio roadmap 與後台刪除補齊：Phase 41 RAG quality regression / DatasetOps 已在 `v0.41.0` 完成 release sync，Phase 42 inference gateway / capacity planning 已在 `v0.42.0` 完成 release sync，Phase 43 AgentOps governance / secure tool runtime 已在 `v0.43.0` 完成 release sync，Phase 44 Document Intelligence QA / human review loop 已在 `v0.44.0` 完成 release sync，Phase 45 final interview portfolio pack 已在 `v0.45.0` 完成 release sync，Phase 46 Admin Document Delete Flow 已在 `v0.46.0` 完成 release sync。這不代表 production annotation platform、model training、full layout understanding、production deployment、production deletion governance 或 JD 100% guarantee 已完成。後續仍需遵守 ticket-first、scope / out-of-scope 與 release sync 規範。
...
後續 roadmap 已新增 Phase 31 到 Phase 46，並建立對應 ticket backlog；目前 Phase 31 已在 `v0.31.0` 完成 PostgreSQL / schema / repository foundation release sync，Phase 32 已在 `v0.32.0` 完成 formal Auth / RBAC / tenant boundary release sync，Phase 33 已在 `v0.33.0` 完成 Redis + NATS worker demo milestone release sync，Phase 34 已在 `v0.34.0` 完成 scanned PDF OCR baseline release sync，Phase 35 已在 `v0.35.0` 完成 RAG indexing quality hardening release sync，Phase 36 已在 `v0.36.0` 完成 eval dashboard / rerank analysis release sync，Phase 37 已在 `v0.37.0` 完成 inference ops / vLLM serving release sync，Phase 38 已在 `v0.38.0` 完成 Agent runtime hardening release sync，Phase 39 已在 `v0.39.0` 完成 deployment / observability / fine-tuning track release sync，Phase 40 已在 `v0.40.0` 完成 JD evidence hardening release sync，Phase 41 已在 `v0.41.0` 完成 RAG quality regression / DatasetOps release sync，Phase 42 已在 `v0.42.0` 完成 inference gateway / capacity planning release sync，Phase 43 已在 `v0.43.0` 完成 AgentOps governance / secure tool runtime release sync，Phase 44 已在 `v0.44.0` 完成 Document Intelligence QA / human review loop release sync，Phase 45 已在 `v0.45.0` 完成 final interview portfolio pack release sync，Phase 46 已在 `v0.46.0` 完成 Admin Document Delete Flow release sync。每個 Phase 都已拆成 contract / runtime / validation / release sync 類型的小票；後續仍必須逐張 ticket 執行，不得自行發明下一個 Phase。
...
- Phase 42 follow-up: `42-03` Streaming Timeout Guardrails 已完成；RAG LLM generation trace 會補上 `llm_timeout_ms`、`llm_num_predict` / `llm_max_tokens`、`llm_streaming_mode`、`llm_truncated_reason` 與 `llm_generation_latency_ms`，timeout path 會回到 retrieved chunks 並標示 `llm_provider_status=timeout`。Validation 已通過：focused backend tests `41 passed`、backend full test script、Phase 42 guardrail keyword `rg` 與 `git diff --check`。不 bump version、不新增 SSE / WebSocket frontend streaming UI、不新增 queue-based scheduler、多使用者 quota、production rate limiter、外部 inference dependency 或預設模型變更。
- Phase 42 follow-up: `42-04` Capacity Planning Report 已完成；新增 `docs/inference-capacity-planning-report.md`，連回 Phase 40 hardware evidence 與 Phase 42 gateway decision，整理 workload profile、capacity table、latency p50 / p95、tokens/sec / throughput、VRAM、KV cache estimate、TOPS / NPU 解讀、fallback policy、skip reason 與模型 / 硬體選型表。Validation 已通過：Phase 42 capacity docs `rg` 與 `git diff --check`。不 bump version、不要求真實 NPU / TOPS profiler、不下載大型模型、不啟動長時間 benchmark、不承諾 production throughput，也不新增 production metrics service 或 autoscaling controller。
- v0.42.0: Inference Gateway / Capacity Planning 已完成；backend package / app version、frontend package / lock / fallback version、health test、Docker Compose `DOCURAG_VERSION`、`.env.example`、demo smoke expected version、README、README_DEV、backend README、frontend README、TODO、ROADMAP 與 ticket 已同步到 `0.42.0`。Phase 42 artifacts 收束為 inference gateway contract、provider routing / fallback metadata、streaming timeout guardrails 與 capacity planning report。Validation 已通過：backend full test、frontend build、inference benchmark smoke skipped report、Phase 42 keyword `rg` 與 `git diff --check`。這是 demo-safe inference ops release，不是 production autoscaling、多 GPU serving、paid API key、production secret、SLA、production metrics service 或 autoscaling controller。
- v0.43.0: AgentOps Governance / Secure Tool Runtime 已完成；backend package / app version、frontend package / lock / fallback version、health test、Docker Compose `DOCURAG_VERSION`、`.env.example`、demo smoke expected version、README、README_DEV、backend README、frontend README、TODO、ROADMAP 與 ticket 已同步到 `0.43.0`。Phase 43 artifacts 收束為 Agent governance contract、tool permission policy metadata、approval fail-closed gate、Agent replay / eval sample 與 report。Validation 已通過：backend full test `269 passed, 1 warning`（pytest cache permission warning）、frontend build、Agent replay smoke、Phase 43 keyword `rg` 與 `git diff --check`。這是 demo-safe Agent governance evidence release，不是 production autonomous Agent、任意 SQL、shell、filesystem command、destructive tool、external side-effect tool、production approval workflow 或 production audit storage。
- v0.44.0: Document Intelligence QA / Human Review Loop 已完成；backend package / app version、frontend package / lock / fallback version、health test、Docker Compose `DOCURAG_VERSION`、`.env.example`、README、README_DEV、backend README、frontend README、TODO、ROADMAP 與 ticket 已同步到 `0.44.0`。Phase 44 artifacts 收束為 field confidence / evidence view、human correction / golden labels、parser field accuracy smoke 與 tracked report。Validation 已通過：backend full test `270 passed, 1 warning`（pytest cache permission warning）、frontend build、parser field accuracy smoke（field accuracy `0.6`、sample count `5`、missing / wrong / evidence mismatch 各 `1`）、Phase 44 keyword `rg` 與 `git diff --check`。這是 demo-safe 文件理解 QA loop，不是 production annotation platform、layout analysis、table reconstruction、model training 或 production OCR accuracy tuning。
- v0.45.0: Final Interview Portfolio Pack 已完成；backend package / app version、frontend package / lock / fallback version、health test、Docker Compose `DOCURAG_VERSION`、`.env.example`、demo smoke expected version、README、README_DEV、backend README、frontend README、TODO、ROADMAP 與 ticket 已同步到 `0.45.0`。Phase 45 artifacts 收束為 JD evidence matrix、system design walkthrough、demo scenario pack 與 risk / tradeoff report。Validation 已通過：backend full test、frontend build、demo smoke、Phase 45 evidence `rg` 與 `git diff --check`。這是 final portfolio documentation release，不是 production guarantee、JD 100% 完成宣稱、新 runtime feature、外部服務、production deployment、模型訓練或 paid API。
- v0.46.0: Admin Document Delete Flow 已完成；backend package / app version、frontend package / lock / fallback version、health test、Docker Compose `DOCURAG_VERSION`、`.env.example`、demo smoke expected version、README、README_DEV、backend README、frontend README、TODO、ROADMAP 與 ticket 已同步到 `0.46.0`。Admin / Analyst 可用 `DELETE /documents/{document_id}` 與後台「刪除文件」按鈕移除文件 metadata、本機 upload artifact 與 PDF page image artifact；Viewer 會被 role guard 擋下。Validation 已通過：focused backend document / auth / health tests、frontend build、Phase 46 keyword `rg` 與 `git diff --check`。這不是 Agent destructive tool、Qdrant cleanup API、restore / recycle bin、production approval workflow、audit storage、worker 清理流程、migration 或 deployment 變更。
- Phase 45 follow-up: `45-01` JD Evidence Matrix 已完成；新增 `docs/jd-evidence-matrix.md`，逐列對照 RAG / embedding / vector DB、rerank metrics、VLM / OCR、SFT / synthetic data、Agent planning、FastAPI / Vue、RBAC / logs / SQL / NoSQL、NATS / gRPC boundary、Redis、vLLM / Ollama、GPU / NPU / KV cache、Docker / K8s 與 validation / honesty boundary。Validation 已通過：JD evidence keyword `rg` 與 `git diff --check`。不 bump version、不新增 runtime、dependency、測試程式、截圖或 demo media。
- Phase 45 follow-up: `45-02` System Design Walkthrough 已完成；新增 `docs/system-design-walkthrough.md`，提供 10 分鐘 talk track、白板式資料流、failure / fallback 解讀與 local JSON vs PostgreSQL、sync API vs worker、demo-safe vs production-ready tradeoff。Validation 已通過：system design keyword `rg` 與 `git diff --check`。不 bump version、不新增 runtime、infra config、deployment service 或架構圖生成工具。
- Phase 45 follow-up: `45-03` Demo Scenario Pack 已完成；新增 `docs/demo-scenario-pack.md`，整理 RAG quality、Document Intelligence QA、AgentOps governance 三條主線，以及 Inference Gateway / capacity planning、Observability evidence、K8s / deployment boundary optional path。Validation 已通過：demo scenario keyword `rg` 與 `git diff --check`。不 bump version、不新增 demo runtime、sample secrets、外部 paid API、optional runtime requirement 或影片媒體。
- Phase 45 follow-up: `45-04` Risk Boundary and Tradeoff Report 已完成；新增 `docs/risk-boundary-and-tradeoff-report.md`，覆蓋 data isolation、DB migration、worker durability、observability、latency、OCR / VLM accuracy 與 Agent safety，逐項列 mitigation、remaining gap、next step 與面試回答。Validation 已通過：risk boundary keyword `rg` 與 `git diff --check`。不 bump version、不新增 runtime safety control、production incident workflow、SLO、pager、external vendor integration 或功能行為變更。
- Phase 44 follow-up: `44-02` Field Confidence and Evidence View 已完成；frontend structured fields surface 顯示 confidence、evidence state、source text、source page、source bbox、parser source 與 fallback reason。Validation 已通過：frontend build、headless Chrome desktop / mobile browser evidence surface、Phase 44 keyword `rg` 與 `git diff --check`。不 bump version、不新增人工修正寫入、golden labels、parser accuracy eval、OCR / VLM provider、parser ranking 變更或 full document image annotation UI。
- Phase 44 follow-up: `44-03` Human Correction and Golden Labels 已完成；backend correction API / metadata 可保存 corrected value、reviewer、reason、version、timestamp 與 source parser value，frontend structured fields surface 可由 Admin / Analyst 保存 golden label，Viewer write API forbidden。`GET /documents/golden-labels` 會匯出 latest golden labels，並新增 `sample-data/eval/parser-golden-labels.json` 作為 demo-safe artifact。Validation 已通過：focused backend tests、frontend build、correction / golden label smoke、Phase 44 keyword `rg` 與 `git diff --check`。不 bump version、不新增 production annotation workflow、multi-review、conflict resolution、external labeling service、model training writeback、production parser prompt writeback 或 destructive edit / delete flow。
- Phase 44 follow-up: `44-04` Parser Field Accuracy Eval 已完成；新增 `scripts/parser-field-accuracy-smoke.ps1`、`sample-data/eval/parser-field-results.json` 與 `sample-data/eval/parser-field-accuracy-report.json`，可重跑 field accuracy / document accuracy / missing field / wrong value / evidence mismatch 報告。Validation 已通過：parser field accuracy smoke、Phase 44 keyword `rg` 與 `git diff --check`。不 bump version、不新增 RAG retrieval eval、LLM-as-judge、model training、production analytics dashboard 或 automatic parser correction。
