...
- Redis、NATS、async worker、queue、production PostgreSQL operation、destructive migration、正式登入、正式權限或 production RBAC。
...
後續 roadmap 已新增 Phase 31 到 Phase 40，並建立對應 future ticket backlog；目前 Phase 31 已在 `v0.31.0` 完成 PostgreSQL / schema / repository foundation release sync，包含 `31-02` PostgreSQL boundary / migration policy、`31-03` DB schema contract、`31-04` repository adapter / migration path 與 `31-05` release sync。建議順序為：`v0.32.0` formal Auth / RBAC / tenant boundary、`v0.33.0` Redis + NATS worker pipeline、`v0.34.0` production OCR / scanned PDF pipeline、`v0.35.0` RAG indexing quality hardening、`v0.36.0` eval dashboard / rerank analysis、`v0.37.0` inference ops / vLLM serving、`v0.38.0` Agent runtime hardening、`v0.39.0` deployment / observability / fine-tuning track、`v0.40.0` JD evidence hardening。每個 Phase 都已拆成 contract / runtime / validation / release sync 類型的小票；Phase 40 則聚焦 Embedding / SFT evidence、inference hardware benchmark evidence 與 observability dashboard evidence。後續仍必須逐張 ticket 執行，不得提前實作下一個 Phase 的 runtime。
