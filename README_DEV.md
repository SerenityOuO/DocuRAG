...
- Phase 34 `34-02` 已完成 demo-safe PDF rendering page image pipeline：新增 `PyMuPDF` dependency，scanned PDF 會產生 bounded PNG page images 與 metadata；mixed PDF 保留 text pages 的 `pdf_text` chunks，並只為 scanned pages 建立 `pdf_mixed_pending_ocr` page images。OCR worker、retry / page-level OCR status 與 release sync 留到 `34-03` / `34-04`。
...
- PDF rendering 已有 demo-safe page image pipeline；仍不執行 scanned PDF OCR、不做 image preprocessing、版面分析、多頁 OCR retry/status runtime 或 OCR accuracy tuning。
...
- LLM-as-judge、answer faithfulness scoring、production eval dashboard、自訂 eval dataset、streaming UI、OpenAI API 或 vLLM serving。
...
5. 在後台觸發 VLM-first parser 後執行 Agent trace demo，說明 `vlm_invoice` / `deterministic_invoice` fallback 如何保存 structured fields，再由 deterministic planner 用 `get_document_fields`、`search_documents`、`summarize_invoice_fields` 形成 source-backed final answer。
...
後續 roadmap 已新增 Phase 31 到 Phase 40，並建立對應 future ticket backlog；目前 Phase 31 已在 `v0.31.0` 完成 PostgreSQL / schema / repository foundation release sync，Phase 32 已在 `v0.32.0` 完成 formal Auth / RBAC / tenant boundary release sync，Phase 33 已在 `v0.33.0` 完成 Redis + NATS worker demo milestone release sync，Phase 34 `34-01` scanned PDF OCR contract 與 `34-02` PDF rendering page image pipeline 已完成。建議後續順序為：Phase 34 `34-03` multipage OCR status / retry、`34-04` `v0.34.0` release sync、`v0.35.0` RAG indexing quality hardening、`v0.36.0` eval dashboard / rerank analysis、`v0.37.0` inference ops / vLLM serving、`v0.38.0` Agent runtime hardening、`v0.39.0` deployment / observability / fine-tuning track、`v0.40.0` JD evidence hardening。每個 Phase 都已拆成 contract / runtime / validation / release sync 類型的小票；Phase 40 則聚焦 Embedding / SFT evidence、inference hardware benchmark evidence 與 observability dashboard evidence。後續仍必須逐張 ticket 執行，不得提前實作下一個 Phase 的 runtime。
