...
- Redis、NATS、async worker、queue、PostgreSQL、資料庫 schema、正式登入、正式權限或 production RBAC。
...
6. 若面試官想看工程細節，再切到 API docs、smoke script output 或 eval CLI 說明 strategy、fallback state 與 trace metadata。
7. 切到 retrieval eval smoke summary，說明 `case_count=20`、Hit Rate@K、MRR@K、Recall@K、failure count 與 trace metadata count。
8. 補充激進預設：`hybrid_rerank` 現在接到 default `/rag/query` 與 Agent search，`vector`、`vector_rerank`、`hybrid` 仍可用 env 明確切換；eval runner 仍可獨立比較各策略。
