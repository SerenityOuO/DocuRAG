...
- [x] `tasks/phase-44-document-intelligence-qa-human-review/44-03-human-correction-and-golden-labels.md`: Admin / Analyst 可在 structured fields surface 保存 demo-safe human correction，backend 會保存 corrected value、reviewer、reason、version 與 timestamp，並可透過 golden labels export 供後續 parser field accuracy eval 使用。不 bump version。
- [x] `tasks/phase-44-document-intelligence-qa-human-review/44-04-parser-field-accuracy-eval.md`: 新增 parser field accuracy smoke、parser result fixture 與 deterministic report，輸出 field accuracy、document accuracy、missing field、wrong value、evidence mismatch、parser_source 與 confidence bucket。不 bump version。
- [x] `tasks/phase-44-document-intelligence-qa-human-review/44-05-phase-44-release-sync.md`: 完成 `v0.44.0` release sync，更新 backend / frontend / Docker Compose / `.env.example` / health test version，並同步 README / README_DEV / backend README / frontend README / TODO / ROADMAP；整理 field confidence / evidence view、human correction / golden labels 與 parser field accuracy eval validation。
...
- `44-03` validation 已通過：focused correction / golden label backend tests、frontend build、correction / golden label smoke、Phase 44 keyword `rg` 與 `git diff --check`。Role guard 保持 Admin / Analyst 可寫、Viewer forbidden；本 ticket 不新增 production annotation workflow、多人審核、conflict resolution、外部 labeling service、model training writeback、production parser prompt writeback 或 destructive edit / delete flow。
- `44-04` validation 已通過：`powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\parser-field-accuracy-smoke.ps1` 通過，summary 為 field accuracy `0.6`、sample count `5`、missing field / wrong value / evidence mismatch 各 `1`；tracked report 以同一支 script 重生；Phase 44 keyword `rg` 與 `git diff --check` 通過。本 ticket 不新增 RAG retrieval eval、LLM-as-judge、model training、production analytics dashboard 或 automatic parser correction。
